...
\fixnote{Need more teaching note about the purposes for this class: (1) For a given area, square will have least perimeter; (2) for a given perimeter, square will have maximum area; (3) limiting cases; (4) what is a function of what; (5) recognizing the type of function from the form of the expression.
Need to mention that rational functions are quotients of polynomial functions (definition).  }

\fixnote{Reorganize so that connecting the dots is present in the first problem and so that function ideas arise in the second problem.  Also add teaching notes about the various graphs that come up, including the ``sideways'' perimeter graph, which is a function, though not displayed conventionally.  Asking ``Is perimeter a function of length?'' is a better question than ``Is this a function?''}
...
\fixnote{Make clear that point that this can be done at many different grade levels.  Edge pieces are merely to get them thinking about perimeter without telling them.}
